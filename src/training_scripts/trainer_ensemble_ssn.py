...
from src.models.ensemble_ssn_lightning_module import EnsembleSSNLightning
from src.callbacks.save_model_callback import SaveModelCallback
...
def main(hparams):
    ...
    # Separate Groups in Wandb for diagonal LSN
    if config["diagonal"]:
        group_name = "Diagonal Ensemble SSN"
    else:
        group_name = "Ensemble SSN"
    ...
    model = EnsembleSSNLightning(config, training_run_name=wandb_logger.name)
