...
class EnsembleLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for idx, model in enumerate(self.model.models):
            ...
            ensemble_logits.append(outputs.detach())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for logit in outputs:
            ensemble_loss.append(self.criterion(logit, targets))
        loss = torch.stack(ensemble_loss).mean()
        mean_sigmoid = torch.stack([torch.sigmoid(logit) for logit in outputs]).mean(
            dim=0
        )
        iou_score = iou(targets.detach(), mean_sigmoid.detach().ge(0.5))
    ...
    def test_step(self, batch, batch_idx):
        ...
        logit_list = self(inputs)
        mean_sigmoid = torch.stack([torch.sigmoid(logit) for logit in logit_list]).mean(
            dim=0
        )
        total_uncertainty = self.model.total_uncertainty(
            inputs, self.hparams.logit_samples
        )
        ...
        prediction_list = [mean_sigmoid.ge(0.5)]
        ...
        ncc_logits = normalized_cross_correlation(
            torch.squeeze(pixelwise_entropy(mean_sigmoid.detach()), dim=1),
            torch.var(torch.squeeze(torch.stack(targets_list).float(), dim=2), dim=0),
        )
