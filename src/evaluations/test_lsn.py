...
from src.models.lsn_lightning_module import LSNLightning
...
def main(hparams):
    ...
    model = LSNLightning(config, training_run_name=config["run_name"])
    model.load_state_dict(checkpoint["state_dict"])
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model.hessian = checkpoint["hessian"].to(device)

    print(model.hessian.device)
